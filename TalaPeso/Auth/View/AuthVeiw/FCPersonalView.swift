...
class FCPersonalView: UIView {
    ...
    var block1: ((FCWenBenCell, ExceptModel) -> Void)?//normal
    
    var block2: ((FCWenBenCell, ExceptModel) -> Void)?//city
...
extension FCPersonalView: UITableViewDelegate, UITableViewDataSource {
    ...
    func popEnumViewWithModel(_ cell: FCWenBenCell, _ model: ExceptModel) {
        guard let ourselves = model.ourselves, let conceive = model.conceive else { return }
        switch (ourselves, conceive) {
        case ("f1", "chance"):
